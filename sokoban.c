...
double time_diff(struct timeval *end, struct timeval *start); // 끝-시작 시간차 리
턴
void undo();
...
void main(){
    ...
    while(1){
        ...
        screen_clear();
        while(1){
            for(int i=0; i<MAX_STAGE; i++){ // flag를 조사하여 클리어한 스테이지는
 skip
...
void save()
{
    ...
    save_point = fopen("sokoban.txt", "wt");
    
     fprintf(save_point, "%s", user_name);
     fprintf(save_point, "\n");
...
void file_load();
{
    ...
    char username[10] = {0};
    load = fopen("sokoban.txt", "rt"); // 파일 오픈       
    
    fscanf(load, "%s\n", &username);
     for (int i = 0; i < map_rows; i++){
          fgets(p_map[i], sizeof p_map[i], load);
     }
...
void save_rank(int level){ // 일단은 stage 1부터 구현
    FILE* file;
    FILE* file2;
        ...
        case 'h':
            ...
            if(map[Py][Px] == '$'){ //이동한 곳에 $가 있다면
                for(int i=0; i<gold_count; i++){
                    if((gold_x[i] == Px) && (gold_y[i] == Py)){ // 해당 $의 index>를 찾아내서
                        undo_record(TRUE,i);
                        gold_x[i] = gold_x[i] - 1;           // @와 동일한 방향으>로 이동
                        if((map[gold_y[i]][gold_x[i]] == '#') || //$가 움직인 방향
에 #이나
                           (map[gold_y[i]][gold_x[i]] == '$')){ // $가 있다면
                            gold_x[i] = gold_x[i] + 1;          // $ 및 @ 이동 취>소
                            Px = Px + 1;
                        }
                    }
                }
...
void printMap(){
    ...
    int row, col;
    for(int k=0; k<slot_count; k++){ // @의 이동으로 인해 slot 위치가 공백이 되지>않도록
